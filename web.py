...
# Optional: offload RIFE frame interpolation to a remote GPU machine over SSH.
# Set DANCEMASHUP_REMOTE_RIFE="user@host" (and optionally DANCEMASHUP_RIFE_GPU
# as the GPU index) to enable. Leave unset to run RIFE locally (if available)
# or fall back to non-interpolated rendering.
_remote_rife = os.environ.get("DANCEMASHUP_REMOTE_RIFE", "").strip()
if _remote_rife:
    _rife_gpu = int(os.environ.get("DANCEMASHUP_RIFE_GPU", "0"))
    engine.set_remote_rife(_remote_rife, gpu=_rife_gpu)
...
def download_videos():
    ...
    def _work():
        dl_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              "downloads")
        os.makedirs(dl_dir, exist_ok=True)
        total = len(items)
    ...
    def _work():
        try:
            ...
            queries = [
                ...
                f"{person} {song} focus cam fancam",
            ...
            # Pick top 8 diverse results (more survives the face filter)
            to_download = filtered[:8]
            _progress(0.15, f"Found {len(all_results)} videos ({len(filtered)} in range), downloading top {len(to_download)}...")
            ...
            for idx, item in enumerate(to_download):
                ...
                _progress(0.15 + idx / len(to_download) * 0.25,
                          f"Step 3/6: Downloading {idx+1}/{len(to_download)}: {title[:40]}")
                try:
                    path = engine.download_video(
                        url, dl_dir,
                        progress_cb=lambda p, i=idx: _progress(
            ...
            if ok_count < 2:
                _broadcast("error_event", {"text": f"Only {ok_count} videos downloaded, need at least 2"})
                return
